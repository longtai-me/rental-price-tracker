        ...
        <div className="filters-section glass-panel">
          <div className="filter-row">
            <div className="filter-group">
              <label><MapPin size={32} weight="regular" /> 縣市</label>
            ...
            <div className="filter-group">
              <label><House size={32} weight="regular" /> 物件類型</label>
            ...
            <div className="filter-group" style={{ gridColumn: 'span 2' }}>
              <label><CurrencyDollar  size={32} weight="regular" /> 租金範圍</label>
            ...
            <div className="filter-group">
              <label><Buildings  size={32} weight="regular" /> 房數</label>
            ...
            <div className="filter-group">
              <label><Lightning size={32} weight="regular" /> 水電費計費</label>
          ...
          {utilityBillingType === 'non-official' && (
            <div className="filter-row animate-fade-in" style={{ 
              ...
              <div className="filter-group">
                <label><Lightning size={32} weight="regular" /> 非夏季電費 (元/度)</label>
                <input 
                  type="number" 
                  className="input-field" 
                  placeholder="例如: 5"
              ...
              <div className="filter-group">
                <label><Lightning  size={32} weight="regular" /> 夏季電費 (元/度)</label>
                <input 
                  type="number" 
                  className="input-field" 
                  placeholder="例如: 6.5"
              ...
              <div className="filter-group">
                <label><Lightning  size={32} weight="regular" /> 最高水費 (元/度)</label>
                <input 
                  type="number" 
                  className="input-field" 
                  placeholder="例如: 15"
          ...
          {showAdvancedFilters && (
            <div style={{ marginTop: '1rem', paddingTop: '1rem', borderTop: '1px solid var(--border-color)', display: 'flex', flexDirection: 'column', gap: '1rem' }}>
              <div className="filter-row" style={{ gridTemplateColumns: 'repeat(auto-fit, minmax(200px, 1fr))' }}>
                <div className="filter-group">
                  <label><Ruler  size={32} weight="regular" /> 坪數範圍 (坪)</label>
                  <div style={{ display: 'flex', gap: '0.5rem', alignItems: 'center' }}>
                  ...
                  <div className="card-tags" style={{ marginTop: '0.75rem', display: 'flex', gap: '0.5rem', fontSize: '0.8rem', flexWrap: 'wrap' }}>
                    {!!item.contractFile && (
                      <span style={{ background: 'rgba(16, 185, 129, 0.2)', color: '#059669', padding: '2px 8px', borderRadius: '4px', display: 'flex', alignItems: 'center', gap: '4px' }}>
                        <CheckCircle  size={32} weight="regular" /> 已認證契約
                      </span>
                    )}
                    {!!item.badLandlord && (
                      <span style={{ background: 'rgba(239, 68, 68, 0.2)', color: '#dc2626', padding: '2px 8px', borderRadius: '4px', display: 'flex', alignItems: 'center', gap: '4px', fontWeight: 600 }}>
                        <XCircle  size={32} weight="regular" /> 惡房東避雷
                      </span>
                    )}
                    {!!item.ghostStory && (
                      <span style={{ background: 'rgba(239, 68, 68, 0.2)', color: '#ef4444', padding: '2px 8px', borderRadius: '4px', display: 'flex', alignItems: 'center', gap: '4px', fontWeight: 600 }}>
                        <Lightning  size={32} weight="regular" /> 靈異事件
                      </span>
                    )}
      ...
      {selectedItem && (
        <div className="modal-overlay" onClick={() => setSelectedItem(null)}>
          <div className="modal-content glass-panel animate-fade-in" onClick={e => e.stopPropagation()}>
            <button className="close-btn" onClick={() => setSelectedItem(null)}><X  size={32} weight="regular" /></button>
            ...
            <div className="modal-body">
              <div className="info-grid">
                <div className="info-item">
                  <House className="info-icon"  size={32} weight="regular" />
                  <div>
                    <label>型態/格局</label>
                ...
                <div className="info-item">
                  <Ruler className="info-icon"  size={32} weight="regular" />
                  <div>
                    <label>建坪/單價</label>
                ...
                <div className="info-item">
                  <Buildings className="info-icon"  size={32} weight="regular" />
                  <div>
                    <label>樓層/屋齡</label>
                ...
                <div className="info-item">
                  <Car className="info-icon"  size={32} weight="regular" />
                  <div>
                ...
                <div className="info-item" style={{ gridColumn: '1 / -1' }}>
                  <Buildings className="info-icon" style={{ color: 'var(--primary)' }}  size={32} weight="regular" />
                  <div>
                    <label>刊登者身分</label>
                    <p style={{ display: 'flex', alignItems: 'center', gap: '0.5rem', flexWrap: 'wrap' }}>
                      ...
                      {selectedItem.verificationStatus === 'verified' && (
                        <span style={{ background: 'rgba(16, 185, 129, 0.2)', color: '#059669', padding: '2px 8px', borderRadius: '4px', fontSize: '0.8rem', display: 'flex', alignItems: 'center', gap: '4px' }}>
                          <CheckCircle  size={32} weight="regular" /> 已審核
                        </span>
                      )}
                      {selectedItem.verificationStatus === 'doubtful' && (
                        <span style={{ background: 'rgba(220, 38, 38, 0.2)', color: '#dc2626', padding: '2px 8px', borderRadius: '4px', fontSize: '0.8rem', display: 'flex', alignItems: 'center', gap: '4px' }}>
                          <Warning  size={32} weight="regular" /> 資訊存疑
                        </span>
                      )}
                    </p>
                  </div>
              ...
              <div style={{ marginTop: '2rem', paddingTop: '1.5rem', borderTop: '1px solid var(--card-border)' }}>
                ...
                <div className="info-grid">
                  <div className="info-item">
                    <CheckCircle className="info-icon" style={{ color: 'var(--primary)' }}  size={32} weight="regular" />
                    <div>
                      <label>租期資訊</label>
                  ...
                  <div className="info-item">
                    <CheckCircle className="info-icon" style={{ color: selectedItem.canSubsidize ? 'var(--primary)' : 'var(--text-muted)' }}  size={32} weight="regular" />
                    <div>
                      <label>租屋補助</label>
                  ...
                  <div className="info-item">
                    <CheckCircle className="info-icon" style={{ color: selectedItem.canMoveHuji ? 'var(--primary)' : 'var(--text-muted)' }}  size={32} weight="regular" />
                    <div>
                      <label>入戶籍</label>
                  ...
                  <div className="info-item" style={{ gridColumn: '1 / -1', alignItems: 'flex-start' }}>
                    <Lightning className="info-icon" style={{ color: 'var(--warning)', marginTop: '4px' }}  size={32} weight="regular" />
                    <div>
                      <label>水電瓦斯費</label>
                      <div style={{ color: 'var(--foreground)', fontWeight: 600, display: 'flex', flexDirection: 'column', gap: '0.4rem', marginTop: '0.4rem' }}>
                        <div style={{ display: 'flex', alignItems: 'center', gap: '0.5rem' }}>
                          <Lightning style={{ color: '#f59e0b' }}  size={32} weight="regular" />
                          <span>電費：{
                            selectedItem.electricityBillingType === 'included' ? '包含在房租中' :
                            selectedItem.electricityBillingType === 'taipower' ? '依台電價格' :
                            (selectedItem.electricityPricePerKwh ? `${selectedItem.electricityPricePerKwh} 元/度` : '未填寫') + 
                            (selectedItem.electricitySummerPricePerKwh ? ` 夏季 ${selectedItem.electricitySummerPricePerKwh} 元/度` : '')
                          }</span>
                        </div>
                        <div style={{ display: 'flex', alignItems: 'center', gap: '0.5rem' }}>
                          <Drop style={{ color: '#3b82f6' }}  size={32} weight="regular" />
                          <span>水費：{
                            selectedItem.waterBillingType === 'included' ? '包含在房租中' :
                            selectedItem.waterBillingType === 'taiwater' ? '依台水價格' :
                            (selectedItem.waterPricePerUnit ? `${selectedItem.waterPricePerUnit} 元/度` : '未填寫') + 
                            (selectedItem.waterSummerPricePerUnit ? ` 夏季 ${selectedItem.waterSummerPricePerUnit} 元/度` : '')
                          }</span>
                        </div>
                      </div>
                    </div>
                  ...
                  <div className="info-item" style={{ gridColumn: '1 / -1' }}>
                    <Car className="info-icon" style={{ color: 'var(--text-muted)' }}  size={32} weight="regular" />
                    <div>
                      <label>交通條件</label>
                      <p style={{ color: 'var(--foreground)' }}>
                  ...
                  <div className="info-item" style={{ gridColumn: '1 / -1' }}>
                    <CheckCircle className="info-icon" style={{ color: 'var(--success)' }}  size={32} weight="regular" />
                    <div>
                      <label>提供設備</label>
                      <p style={{ color: 'var(--foreground)' }}>
                  ...
                  <div className="info-item" style={{ gridColumn: '1 / -1' }}>
                    <CheckCircle className="info-icon" style={{ color: 'var(--warning)' }}  size={32} weight="regular" />
                    <div>
                      <label>房屋特色與規定</label>
                      <p style={{ color: 'var(--foreground)' }}>
                        {[
                  ...
                  {selectedItem.badLandlord && (
                    <div className="info-item" style={{ gridColumn: '1 / -1' }}>
                      <XCircle className="info-icon" style={{ color: '#dc2626' }}  size={32} weight="regular" />
                      <div>
                  ...
                  {selectedItem.ghostStory && (
                    <div className="info-item" style={{ gridColumn: '1 / -1' }}>
                      <Lightning className="info-icon" style={{ color: '#ef4444' }}  size={32} weight="regular" />
                      <div>
